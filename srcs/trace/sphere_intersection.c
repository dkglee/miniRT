...
/*   Updated: 2021/05/10 20:46:10 by deulee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		intersection_sphere(double dis[2], t_vec origin, t_vec dir, t_object *list)
{
	...
	oc = ft_vec_sub(origin, list->fig.sp.center);
	a = ft_vec_dot(dir, dir);
	b = 2 * ft_vec_dot(dir, oc);
	c = ft_vec_dot(oc, oc) - pow(list->fig.sp.radius, 2);
...
double			sphere_intersection_point(t_vec origin, t_vec dir, t_object *list)
{
	...
	intersection_sphere(dis, origin, dir, list);
	...
	p1 = ft_vec_add(origin, ft_vec_product(dis[0], dir));
	p2 = ft_vec_add(origin, ft_vec_product(dis[1], dir));
