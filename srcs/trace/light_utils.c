...
/*   Updated: 2021/05/14 17:58:55 by deulee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_light(double (*rgb)[3], double ratio, int color)
{
	...
	mask = 255 << 16;
	(*rgb)[0] += ratio * ((color & mask) >> 16) / 255;
	mask >>= 8;
	(*rgb)[1] += ratio * ((color & mask) >> 8) / 255;
	mask >>= 8;
...
double	apply_specular(t_ray ray, t_inter *inter, t_scene trace, t_object *list)
{
	...
	vec_to_light = ft_vec_sub(trace.light->o, inter->p);
	vec_to_cam = ft_vec_sub(ray.origin, inter->p);
	reflec = reflection(vec_to_light, inter->norm);
