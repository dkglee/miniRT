...
int		super_sample(int *color, t_tmp tmp, t_render *render)
{
	...
	c_color = calc(4, tmp, render);
	i = 0;
	while (i < 4)
	{
		if (!color_difference(color[n], c_color) || tmp.limit == 0)
			color[n] = calc_avg_simple_color(color[n], c_color);
		else
		{
			if (i == 0)
				color[0] = ssaa_zero(color, c_center, tmp, render);
			else if (i == 1)
				color[1] = ssaa_one(color, c_center, tmp, render);
			else if (i == 2)
				color[2] = ssaa_two(color, c_center, tmp, render);
			else
				color[3] = ssaa_three(color, c_center, tmp, render);
		}
		i++;
	}
