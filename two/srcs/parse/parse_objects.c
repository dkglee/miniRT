...
/*   Updated: 2021/04/06 00:49:43 by deulee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_plane(t_parse *parse)
{
	...
	if (!parse_vec(parse->info[0], &new->fig.pl.p) ||
			!parse_vec(parse->info[1], &new->normal) ||
			!parse_int(parse->info[2], &new->specular) ||
			!parse_double(parse->info[3], &new->reflec) ||
			!parse_double(parse->info[4], &new->refrac) ||
			!parse_int(parse->info[5], &new->texture) ||
...
void	parse_triangle(t_parse *parse)
{
	...
	if (!parse_vec(parse->info[0], &new->fig.tr.p1) ||
			!parse_vec(parse->info[1], &new->fig.tr.p2) ||
			!parse_vec(parse->info[2], &new->fig.tr.p3) ||
			!parse_int(parse->info[3], &new->specular) ||
			!parse_double(parse->info[4], &new->reflec) ||
			!parse_double(parse->info[5], &new->refrac) ||
			!parse_int(parse->info[6], &new->texture) ||
...
void	parse_cylinder(t_parse *parse)
{
	...
	if (!parse_vec(parse->info[0], &new->fig.cy.center) ||
			!parse_vec(parse->info[1], &new->fig.cy.nv) ||
			!parse_double(parse->info[2], &new->fig.cy.radius) ||
			!parse_double(parse->info[3], &new->fig.cy.height) ||
